...
import { fetchReposAction, fetchProfileAction } from "./redux/slices/githubSlices";

const App = () => {
  const [user, setUser] = useState('tweneboah');
  const dispatch = useDispatch();
  const { loading, reposList, profile, error } = useSelector(state => state?.repos);
    ...
    <>
      <section className="relative 2xl bg-gray-800 min-h-screen">
        <div className="relative container px-4 mx-auto">
          ...
          {/* Content goes here */}
          {loading ? (
            <h1 className="text-green-300 text-3xl text-center">Loading...</h1> 
            ) : error ? (
              ...
              <div className="flex flex-wrap -mx-4 mb-20">
                <div className="w-full lg:w-1/2 px-4 mb-4 lg:mb-0">
                  <div className="bg-gray-600 rounded-b-lg">
                    ...
                    <div className="px-14 py-8">
                      ...
                      <h4 className="mb-8 lg:mb-4  text-white font-bold">
                        Repositories:{" "}
                        <span classNameName="ml-2 inline-flex items-center px-2.5 py-0.5 rounded-lg text-xs font-medium bg-green-500 text-green-800">
                      ...
                      <div className="md:text-right">
                        <a
                          ...
                          target="_blank"
                          className="inline-block px-12 py-4 border border-gray-300 hover:border-gray-200 rounded-full font-bold text-white" rel="noreferrer"
                        >
                          View Profile
                        </a>
                      </div>
                ...
                <div className="w-full lg:w-1/2 px-4">
                  {/* Loop this */}
                  {reposList?.name !== "Error" && reposList?.map((repo) => (
                    <>
                    <div className="py-6 px-8 mb-4 bg-gray-600 rounded-lg">
                      <div className="flex items-center">
                        <a href={repo?.html_url} target="_blank" className="text-lg  text-indigo-400" rel="noreferrer">
                          {repo?.name}
                        </a>
                      </div>
                    </div>
